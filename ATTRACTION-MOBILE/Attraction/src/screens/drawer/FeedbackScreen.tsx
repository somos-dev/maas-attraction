...
export default function FeedbackScreen() {
  ...
  useEffect(() => {
    if (myFeedback?.text) {
      // Se il feedback inizia con "⭐ X/5", separa il testo successivo
      const match = myFeedback.text.match(/^⭐\s*(\d)\s*\/\s*5\s*\n?(.*)$/is);
      if (match) {
        const parsed = parseInt(match[1], 10);
        if (parsed >= 1 && parsed <= 5) setRating(parsed);
        setComment(match[2]?.trim() || '');
      } else {
        // Nessuna stella trovata → usa il testo intero
        setComment(myFeedback.text.trim());
      }
    }
  }, [myFeedback]);
